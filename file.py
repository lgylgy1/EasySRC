...
def read_files_by_extensions(config, path: str, filter_comment: bool = True) -> Dict[str, List[str]]:
    """
    读取指定路径的内容（文件或文件夹），并将错误信息记录到日志
    
    参数:
        config: Config类实例，需包含 text_extensions 配置
        path: 待读取的文件或文件夹路径
    ...
    # 2. 如果是文件夹，递归读取所有符合条件的文件
        ...
        try:
            ...
            for item in os.listdir(path):
                ...
                # 递归处理子文件夹或文件
                sub_result = read_files_by_extensions(config, item_path)
                # 合并结果（跳过错误信息，只保留文件内容）
